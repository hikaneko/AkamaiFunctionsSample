...
cd AkamaiFunctionsSample/zuplo-ai-gateway-integration
npm install
```

### 2. Configure your Zuplo Gateway URL

Edit `src/index.js` and replace the placeholder with your Zuplo AI Gateway endpoint:

```js
const UPSTREAM_URL = "https://<your-zuplo-gateway>/v1/chat/completions";
```

Also update `spin.toml` to reflect your gateway hostname in `allowed_outbound_hosts`:

```toml
allowed_outbound_hosts = ["https://<your-zuplo-gateway>", "http://edc.edgesuite.net"]
```
...
> **Note:** `npm install` automatically patches a known issue in `componentize-js` where paths containing spaces cause the build to fail (the `postinstall` script handles this). The `postinstall` script uses macOS-style `sed -i ''`; on Linux, remove the empty string argument.
...
The browser sends requests to `/api/chat` with an `Authorization: Bearer <api-key>` header. The Wasm Function forwards the request body and authorization header to your Zuplo gateway and streams the response back — the upstream URL is never exposed to the browser.
